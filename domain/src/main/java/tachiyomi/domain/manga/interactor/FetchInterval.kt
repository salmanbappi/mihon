...
class FetchInterval(
    ...
    private fun calculateNextUpdate(
        ...
        if (manga.fetchIntervalDays != 0) {
            return calculateScheduledUpdate(manga, dateTime, timeZone)
        }
    ...
    private fun calculateScheduledUpdate(
        manga: Manga,
        dateTime: LocalDateTime,
        timeZone: TimeZone,
    ): Long {
        ...
        var next = LocalDateTime(nextDate.year, nextDate.monthNumber, nextDate.dayOfMonth, hour, minute, 0, 0)
        if (next < dateTime) {
            nextDate = nextDate.plus(1, DateTimeUnit.DAY)
            next = LocalDateTime(nextDate.year, nextDate.monthNumber, nextDate.dayOfMonth, hour, minute, 0, 0)
        }

        while (true) {
            val dayOfWeek = next.dayOfWeek.isoDayNumber // 1 (Mon) to 7 (Sun)
            val bit = 1 shl (dayOfWeek - 1)
            if (days and bit != 0) {
                return next.toInstant(timeZone).toEpochMilliseconds()
            }
            nextDate = nextDate.plus(1, DateTimeUnit.DAY)
            next = LocalDateTime(nextDate.year, nextDate.monthNumber, nextDate.dayOfMonth, hour, minute, 0, 0)
        }
